...
function Arrow() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="hidden h-5 w-5 shrink-0 text-[#d2d2d7] sm:block" aria-hidden>
...
export function FunnelRow({ clicks, leads, converted }: { clicks: number; leads: number; converted: number }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
      <h2 className="text-sm font-semibold text-[#1d1d1f]">Funil de conversão</h2>
      <div className="mt-4 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center">
        {stages.map((stage, i) => (
          <div key={stage.label} className="flex flex-1 items-center gap-4">
            {i > 0 && <Arrow />}
            <div className="flex-1 rounded-xl bg-[#f5f5f7] px-4 py-3">
              <p className="text-xs font-medium text-[#86868b]">{stage.label}</p>
              <p className="mt-0.5 text-2xl font-semibold tracking-tight text-[#1d1d1f]">{stage.value}</p>
              <p className="text-xs text-[#86868b]">{stage.sub}</p>
            </div>
          </div>
        ))}
      </div>
